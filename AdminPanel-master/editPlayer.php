...
echo '<center><h4>Civ-Cash: $'.$player->civ_cash.'</h3></center>';
echo '<center><h4>Civ-Bank: $'.$player->civ_bankacc.'</h3></center>';
...
echo '<center><h4>Civ-Stats: '.$player->civ_stats.'</h3></center>';
echo '<center><h4>Civ-Position: '.$player->civ_alive.'</h3></center>';
echo '<center><h4>Civ-Alive: '.$player->civ_position.'</h3></center>';
echo '<center><h4>Blacklist: '.$player->blacklist.'</h3></center>';
echo '<center><h4>Experience: '.$player->experience.'</h3></center>';
echo '<center><h4>Loadout: '.$player->loadout.'</h3></center>';
echo '<center><h4>Arrested: '.$player->arrested.'</h3></center>';
echo '<center><h4>Arresttime: '.$player->arrestTime.'</h3></center>';
echo '<center><h4>First seen: '.$player->first_join.'</h3></center>';
echo '<center><h4>Licenses: '.$player->civ_licenses.'</h3></center>';
echo '<center><h4>Playtime: '.$player->playtime.'</h3></center>';
...
<div class='panel panel-info'>
  <div class='panel-heading'>
    <h3 class='panel-title'>Medic Information</h3>
  </div>
  <div class='panel-body'>
      <?php
      echo "<center><h4>Med-Cash: $$player->med_cash</h4></center>
      <center><h4>Med-Bank: $$player->med_bankacc</h4></center>
      <center><h4>Mediclevel: $player->mediclevel</h4></center>
      <center><h4>Stats: $player->med_stats</h4></center>
      <center><h4>Licenses: $player->med_licenses</h4></center>
      <center><h4>Alive: $player->med_alive</h4></center>
      <center><h4>Position: $player->med_position</h4></center>
      <center><h4>Loadout: $player->med_loadout</h4></center>";
      ?>
...
<div class='panel panel-info'>
    <div class='panel-heading'>
        <h3 class='panel-title'>Cop Information</h3>
    </div>
    <div class='panel-body'>
        <?php
        echo "<center><h4>Cop-Cash: $$player->cop_cash</h4></center>
      <center><h4>Cop-Bank: $$player->cop_bankacc</h4></center>
      <center><h4>Coplevel: $player->coplevel</h4></center>
      <center><h4>Stats: $player->cop_stats</h4></center>
      <center><h4>Licenses: $player->cop_licenses</h4></center>
      <center><h4>Alive: $player->cop_alive</h4></center>
      <center><h4>Position: $player->cop_position</h4></center>
      <center><h4>Loadout: $player->cop_loadout</h4></center>";
        ?>
    </div>
</div>
    ...
    <div class='panel-body'>
        <div class="table-responsive">
            <table class="table table-striped" style = "margin-top: -10px">
                ...
                <tbody>
                    <?php
                    $sqlvehicles = "SELECT * FROM vehicles WHERE pid='$pid';";
                    $resutlvehicles = mysqli_query($dbcon, $sqlvehicles) or die('Connection could not be established');
                    while ($row = mysqli_fetch_array($resutlvehicles, MYSQLI_ASSOC)) {
                        echo '<tr>';
                        echo '<td>' . $row['id'] . ' </td>';
                        echo '<td>' . $row['side'] . ' </td>';
...
<div class='panel panel-info'>
    <div class='panel-heading'>
        <h3 class='panel-title'>Player Logs</h3>
    </div>
    <div class='panel-body'>
        <?php
        $search = $username;
        // Read from file
        $lines = file('serverlog.log');
        ...
        //        $fh = fopen('serverlog.log','r');
//        while ($line = fgets($fh)) {
//            if($line.contains($usernae)){
//                echo $username;
//            }
//
//        }
